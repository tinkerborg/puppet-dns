# == Define: bind_dns::record::ns
#
# Wrapper of bind_dns::record to set NS records
#
define bind_dns::record::ns (
  $zone,
  $data,
  ...
  if !is_domain_name($zone) or $zone =~ /^[0-9\.]+$/ {
    fail("Define[bind_dns::record::ns]: NS zone ${zone} must be a valid domain name.")
  }
  ...
  if !is_domain_name($data) or $data =~ /^[0-9\.]+$/ {
    fail("Define[bind_dns::record::ns]: NS data ${data} must be a valid hostname.")
  }
