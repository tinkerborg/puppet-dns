# == Define: bind_dns::record::mx
#
# Wrapper for bind_dns::record to set an MX record.
#
define bind_dns::record::mx (
  $zone,
  $data,
  ...
  if !is_domain_name($zone) or $zone =~ /^[0-9\.]+$/ {
    fail("Define[bind_dns::record::mx]: MX zone ${zone} must be a valid domain name.")
  }
  ...
  if !is_domain_name($data) or $data =~ /^[0-9\.]+$/ {
    fail("Define[bind_dns::record::mx]: MX data ${data} must be a valid hostname.")
  }
  if !is_integer($preference) or $preference < 0 or $preference > 65536 {
    fail("Define[bind_dns::record::mx]: preference ${preference} must be an integer within 0-65536.")
  }
  if !is_domain_name($host) and $host != '@' {
    ...
    fail("Define[bind_dns::record::mx]: MX host label ${host} must be a valid hostname or '@' to signify \$ORIGIN.")
  }
